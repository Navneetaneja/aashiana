...
class Aashiana extends React.Component {
  ...
  check(d, m, y) {
    ...
    if (d === 9 && m === 10) {
      ...
      this.me = "Click My Pic !!";
      this.desc =
        "WISHING HAPPY BIRTHDAY TO THIS SPECIAL LADY (#nani,#dadi,#mom,#saas,#wife)\nCONGRATULATIONS!!";
    } else if (d === 8 && m === 4) {
  ...
  tags() {
    if (this.wish !== "") {
      return (
        <div>
          <h3 className="text-danger bg-warning font-italic progress-bar-striped progress-bar-animated">
            <marquee direction="left">{this.wish}</marquee>
          </h3>
          <h3 className="text-white font-italic bg-danger progress-bar-striped progress-bar-animated">
            {this.to}
          </h3>
          <h3 className="text-success font-italic bg-warning progress-bar-striped progress-bar-animated">
            <marquee direction="right">{this.name}</marquee>
          </h3>
        </div>
      );
    } else {
